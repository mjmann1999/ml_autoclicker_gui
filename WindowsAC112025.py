...
def payload_to_settings(payload: dict, default: Settings) -> Settings:
    ...
    legacy_dynamic_keys = [
        "dynamic_std_enabled", "interval_std_gain_per_min", "interval_std_max",
        "duration_std_gain_per_min", "duration_std_max",
        "std_increase_rate_interval_std", "std_max_interval_std",
        "std_increase_rate_duration_std", "std_max_duration_std",
    ]
...
class AutoclickerThread(QThread):
    ...
    def _update_status(self):
        ...
        if not running or not start_time:
            self._emit_status("Stopped", "Press Start or Hotkey")
...
class AutoclickerGUI(QWidget):
    toggle_request_signal = pyqtSignal()  # Signal to handle hotkey event in GUI thread

    def __init__(self, settings: Settings):
        super().__init__()
        self.settings_obj = settings
        self.profiles = {}
        self.setWindowTitle("Windows ML Autoclicker")
        ...
        # Hotkey Signal connection
        self.toggle_request_signal.connect(self._handle_hotkey_toggle)

        # Load last hotkey
        saved_key = self._settings.value("hotkey", "F6")
        self.txt_hotkey.setText(str(saved_key))
        self._update_hotkey()
    ...
    def _build_tab_main(self, w: QWidget):
        ...
        # Hotkey Section
        hk_group = QGroupBox("Global Hotkey")
        hk_layout = QHBoxLayout(hk_group)
        self.txt_hotkey = QLineEdit("F6")
        self.txt_hotkey.setPlaceholderText("e.g. F6, Ctrl+Shift+X")
        self.btn_set_hotkey = QPushButton("Set Hotkey")
        hk_layout.addWidget(QLabel("Start/Stop Key:"))
        hk_layout.addWidget(self.txt_hotkey)
        hk_layout.addWidget(self.btn_set_hotkey)
        v.addWidget(hk_group)
        
        # Connect button
        self.btn_set_hotkey.clicked.connect(self._update_hotkey)
    ...
    def _update_hotkey(self):
        hotkey_str = self.txt_hotkey.text().strip()
        if not hotkey_str:
            return
        ...
        try:
            ...
            # Add new hook
            keyboard.add_hotkey(hotkey_str, self._on_hotkey_press)
            
            self._settings.setValue("hotkey", hotkey_str)
            self._append_log(f"Hotkey set to: {hotkey_str}")
        except ImportError:
            self._append_log("Error: 'keyboard' module not found. Hotkeys disabled.")
        except Exception as e:
            self._append_log(f"Invalid hotkey: {e}")
            QMessageBox.warning(self, "Hotkey Error", f"Could not register '{hotkey_str}'.\nTry 'F6' or 'Ctrl+Shift+A'.")

    def _on_hotkey_press(self):
        # This runs in keyboard's thread. Must emit signal to GUI thread.
        self.toggle_request_signal.emit()
    ...
    def _on_quit(self):
        try:
            keyboard.unhook_all_hotkeys() # Clean up hooks
            self.thread.stop_clicking()
